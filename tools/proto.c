...
void *proto_json_search(proto_cache_t *c, const char *path)
{
	...
	while((p_path = proto_next_node(p_path,node_name))){
		//printf("node_name %s\n", node_name);
		if(cJSON_IsArray(m)){
			cJSON_ArrayForEach(tmp, m){
				n = cJSON_GetObjectItem(tmp,node_name);
				if(n && n->type == cJSON_String
					 && strcmp(n->string, node_name) == 0)
					break;
			}
		}else{
...
proto_cache_t *proto_protobuf_init(void *old_cache, char *context)
{
	// placeholder
	return NULL;
}

proto_cache_t *proto_xml_init(void *old_cache, char *context)
{
	// placeholder
	return NULL;
}

proto_cache_t *proto_ini_init(void *old_cache, char *context)
{
	// placeholder
	return NULL;
}

int proto_protobuf_destroy(proto_cache_t *c)
{
	// placeholder
	return 0;
}

int proto_xml_destroy(proto_cache_t *c)
{
	// placeholder
	return 0;
}

int proto_ini_destroy(proto_cache_t *c)
{
	// placeholder
	return 0;
}
